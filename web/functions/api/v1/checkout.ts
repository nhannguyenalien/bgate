...
export async function onRequestPost({ request, env }: Context): Promise<Response> {
  ...
  const provider = body.provider === "crypto" ? "btcpay" : String(body.provider || "");
  if (!body.product || !body.user_id || !["btcpay", "whop", "gumroad"].includes(provider)) return json({ error: "product, user_id and valid provider are required" }, 422);
  ...
  try {
    ...
    const rows = await sql`UPDATE orders SET provider_payment_id = ${checkout.paymentId}, checkout_url = ${checkout.checkoutUrl}, status = ${checkout.status}::paymentstatus, updated_at = now() WHERE id = ${id}::uuid RETURNING *`;
